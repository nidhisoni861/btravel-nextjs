      ...
      <div className="max-w-[1200px] mx-auto px-8 py-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="mb-16">
          <div className="flex items-center gap-4 mb-4">
            <FileText className="w-10 h-10 text-[var(--champagne)]" />
            <h1 className="font-serif text-5xl text-[var(--navy)]">My Documents</h1>
          </div>
          <p className="text-[var(--navy)]/70 text-lg">All your booking confirmations, invoices and travel documents</p>
        </motion.div>
              ...
              <motion.div key={doc.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}
                ...
                <div className="flex items-start gap-6">
                  <div className="flex-shrink-0">
                    <div className="w-16 h-16 bg-[var(--sand-light)] flex items-center justify-center">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-serif text-xl text-[var(--navy)] mb-1">{doc.type}</h3>
                        <p className="text-[var(--navy)]/70 mb-2">{doc.tripName}</p>
                        <div className="flex items-center gap-4 text-sm text-[var(--navy)]/60">
                          <span className="flex items-center gap-2"><FileText className="w-4 h-4" />{doc.bookingNumber}</span>
                          <span className="flex items-center gap-2"><Calendar className="w-4 h-4" />{doc.date}</span>
                      ...
                      <span className={`px-3 py-1 text-sm font-medium ${doc.statusColor} bg-white border-2 border-current`}>{doc.status}</span>
                    </div>
                    <div className="flex gap-3 mt-4">
                      <button onClick={() => alert(`Preparing download: ${doc.type} - ${doc.bookingNumber}`)}
                        className="flex items-center gap-2 px-6 py-2 bg-[var(--champagne)] text-white hover:bg-[var(--champagne)]/90 transition-colors">
                        <Download className="w-4 h-4" /><span>Download PDF</span>
                      </button>
                      <button onClick={() => alert(`${doc.type} sent via email`)}
                        className="flex items-center gap-2 px-6 py-2 border-2 border-[var(--sand)] text-[var(--navy)] hover:border-[var(--champagne)] transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }}
          className="mt-12 bg-[var(--sand-light)] border-2 border-[var(--sand)] p-8">
