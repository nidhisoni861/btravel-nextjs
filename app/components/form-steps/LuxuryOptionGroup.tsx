    ...
    <div>
      ...
      <div className="space-y-2">
        {options.map((option) => {
          ...
          return (
            <label key={option.key} className="flex items-center gap-3 p-4 border-2 border-[var(--sand)] hover:border-[var(--champagne)] transition-all cursor-pointer group">
              ...
              <button type="button" onClick={(e) => { e.preventDefault(); onToggle(option.key as keyof FormData['luxuryToggles']); }}
                className={`w-5 h-5 border-2 flex items-center justify-center transition-all ${isChecked ? 'bg-[var(--champagne)] border-[var(--champagne)]' : 'bg-white border-[var(--sand)] group-hover:border-[var(--champagne)]'}`}>
